...
from .configuration import whim_stan_config
...
whimstan_console_log_handler = RichHandler(
    level=whim_stan_config.logging.level,
    rich_tracebacks=True,
    markup=True,
    console=console,
)
whimstan_console_log_handler.setFormatter(_console_formatter)
...
def silence_warnings():
    ...
    whimstan_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    whimstan_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    whimstan_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    if whim_stan_config.logging.on:

        # add the handlers
        log.addHandler(whimstan_console_log_handler)
