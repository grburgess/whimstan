...
class WCSCircleInsetConnectionPatch(PathPatch):
    ...
    def get_path(self):
        ...
        return Path(
            np.row_stack((p0, p1)), np.asarray([Path.MOVETO, Path.LINETO])
        )
...
class AutoScaledWCSAxes(WCSAxes):
    ...
    def mark_inset_circle(self, ax, center, radius, *args, **kwargs):
        ...
        """
        center = SkyCoord(
            center, representation_type=UnitSphericalRepresentation
        ).icrs
        ...
        for ax in (self, ax):
            ax.add_patch(
                SphericalCircle(
                    *args, **kwargs, transform=ax.get_transform("world")
                )
            )
