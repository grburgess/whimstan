...
real partial_log_like_absori(int [] n_slice, int start, int end, int N_ene, int N_chan, vector[] host_precomputed_absorp, vector[] precomputed_absorp, vector[] ene_avg, vector[] ene_width, int[,] mask, int[] n_chans_used, vector K, vector index, vector nH_host, vector nH_mw, real n0, matrix num, matrix[,] sum_sigma_interp,  matrix[] rsp , vector exposure, vector exposure_ratio, vector[] counts, vector[] bkg){
  int slice_length = num_elements(n_slice);
  ...
  for (i in 1:slice_length){
    ...
    vector[N_ene] source_spectrum = powerlaw_flux(ene_avg[n], K[n], index[n], 0.4, 15) .* absorption(nH_host[n], host_precomputed_absorp[n]) .* absorption(nH_mw[n], precomputed_absorp[n]) .* integrate_absori(sum_sigma_interp[n], num, n0, N_ene);
