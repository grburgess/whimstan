...
model{
  ...
  target += reduce_sum(partial_log_like_whim_and_mw_t_fixed, all_N, grainsize, N_ene, N_chan, ene_avg, ene_width, mask, n_chans_used, mw_abs, K, index, n0_whim, whim_abs_base, rsp, exposure, exposure_ratio, counts, bkg);
...
generated quantities {
	  ...
	  for (i in 1:N_grbs){
	      ...
	      source_spectrum[i] = powerlaw_spectrum[i] .* whim_abs[i] .* mw_abs[i];
